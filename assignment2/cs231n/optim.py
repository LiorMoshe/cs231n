...
def adam(x, dx, config=None):
    ...
    unbiased_first = first_moment / (1 - config['beta1'] ** config['t'])
    second_moment = config['beta2'] * config['v'] + (1 - config['beta2']) * dx * dx
    unbiased_second = second_moment / (1 - config['beta2'] ** config['t'])
    next_x = x - config['learning_rate'] * unbiased_first / (np.sqrt(unbiased_second) \
                                                    + config['epsilon'])
